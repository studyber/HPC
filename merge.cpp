...
void mergesort(int a[],int i,int j){
    ...
    if(i<j){
        mid=(i+j)/2;
        // pragma is compiler directive 
		// omp parallel sections lets you define parallel region containing a single section 
...
int main(){
    ...
    // Parallel algorithm
    //start_time = omp_get_wtime();
	// explicitly tells compiler to parallelize the chosen block of code
    #pragma omp parallel
    {
		// code must be executed only once by any thread from team 
        #pragma omp single
        {
            mergesort(b, 0, n-1);
        }
